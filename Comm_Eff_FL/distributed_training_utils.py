...
import time
from SGD import StepAHeadErrorFeedbackSGD 
from SGD import LocalErrorSGD
...
class Client(DistributedTrainingDevice):
  ...
  def __init__(self, dataloader, model, hyperparameters, experiment, id_num=0):
    ...
    self.bits_sent = []
    comp = 'scaled_sign'
    optimizer = StepAHeadErrorFeedbackSGD(self.model.parameters(), lr=0.01, momentum=0.9, weight_decay=0.0,
                                 comp=comp)

    optimizer_local_error = LocalErrorSGD(model.parameters(),
                      lr=0.01,
                      gmf=0,
                      tau=98,
                      size=8,
    ...
    #here we can call the which algorithm need for optimizing the model    
    #self.optimizer =    optimizer_object(self.model.parameters(), **optimizer_parameters) #
    self.optimizer = optimizer#-->(StepHeaderrorfeedback with momentem SGD)
    #self.optimizer =optimizer_local_error
  ...
  #This function for calling the compression method 
...
class Server(DistributedTrainingDevice):
  ...
  def __init__(self, dataloader, model, hyperparameters, experiment, stats):
    ...
    self.client_sizes = torch.Tensor(stats["split"]).cpu()
