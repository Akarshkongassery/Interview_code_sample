...
def run_experiments(experiments):
  ...
  for xp_count, xp in enumerate(experiments):
    ...
    # Instantiate Clients and Server with Vgg11*
    
    net = getattr(neural_nets, hp['net']) #call the model using hyper parameter 
    ...
    with open('results.csv', 'w', newline='') as file:
      ...
      field = ["epoch", "communication_round", "iteration","train_acc","test_acc","train_loss","test_loss","CommunicationCost","CommunicationTime"]
      ...
      for c_round in range(1, hp['communication_rounds']+1):
        #Call the participent clients randomly
        participating_clients = random.sample(clients, int(len(clients)*hp['participation_rate'])) 

        # Clients do
        for client in participating_clients:
          #client has connected with server 
          client.synchronize_with_server(server)
          #training process , wights update will be done in below function for client side 
          client.compute_weight_update(hp['local_iterations'])
          #weight Compression using Compression Techniques like Topk,sgd etc...
          client.compress_weight_update_up(compression=hp['compression_up'], accumulate=hp['accumulation_up'], count_bits=hp["count_bits"])

        # Server does the combine the weights and update the weights .  
        ...
        if xp.is_log_round(c_round):
          ...
          Communication_cost=Communication_cost+default_cost
          # Timing
          ...
          writer.writerow([clients[0].epoch,c_round,c_round*hp['local_iterations'],train_values[1],test_values[1],train_values[0],test_values[0],Communication_cost,avrg_time_per_c_round])
